...
In early 2018 I noticed the market getting flooded by cheap Android TV boxes. I quickly [bought one](https://www.amazon.com/Android-Leelbox-Smart-Support-2-4GHz/dp/B07FNZJYYY/ref=cm_cr_arp_d_product_top?ie=UTF8), mainly due to the fact that our old Philips TV was not Android based yet. I was also curious and wanted to see what kind of experience these boxes deliver. Long story short, I think I only booted it a handful of times :-)
...
There were several different models of the Leelbox Q4 floating around Amazon and AliExpress, namely Q4, Q4 S, and Q4 Max. The main difference seems to be in the amount of the internal storage (16, 32, and 64GB).
...
My particular model is called Q4 S, and it packs a 4GB of DDR3 RAM and a 32GB of eMMC storage. As far as connectivity goes, it offers 1xHDMI, 2xUSB 2.0, 1xUSB 3.0, 100M Ethernet, 2.4GHz WiFi, and BlueTooth 4.1.
...
- A class A1 micro SDHC card (mine was a Sandisk Ultra 32GB) and a micro SD card reader
...
- A local ETH network with a DHCP service

- A HDMI cable & USB keyboard are optional, but quite handy for debugging and initial setup 
...
First of all, download the armbian image for the RK3288-based TV box.  [Here](https://mega.nz/folder/zoZywCTQ#9sNfXIAB3m_DyLl5I8P94w/file/btpyQCzY) is the link to custom images maintained by the armbian Forum user [balbes150](https://forum.armbian.com/topic/8082-armbian-for-tv-box-rk3328/). 
...
Based on a little bit of error and trial (definitely much less than I anticipated), I found that the  [rk3328-t9-mainline.dtb](files/rk3328-t9-mainline.dtb) works the best for my box.

| DTB                                                    | Target system                         | Source                                                       | Result of using on my box                                    |
| ------------------------------------------------------ | ------------------------------------- | ------------------------------------------------------------ | ------------------------------------------------------------ |
| [rk3328-box-h96mp.dtb](files/rk3328-box-h96mp.dtb)     | H96 Max+                              | [easyb](https://forum.armbian.com/topic/8082-armbian-for-tv-box-rk3328/?do=findComment&comment=94857) from armbian forum | System boots when but several devices are missing, e.g.      |
...
Insert your SD card into the PC and use Balena Etcher or a similar tool to flash the image to the card. 
...
2. Edit the `uEnv.txt` file to instruct the bootloader to pass this DTB file to the kernel.
   ...
   Make sure that the FDT line points to the your DTB file! 
   
   *The file is using Unix line ending (\n), in case your editor is confused. On Windows I recommend Notepad++.*
...
If all goes well, your box should boot from the SD card once given power! After I saw the ETH light on the switch turning on,  I went to the web interface of our home router to check the DHCP server log to find out the IP the box.
...
- It may be necessary connect a USB keyboard and a HDMI TV or monitor for debugging.
...
**6. Initial configuration**
 ...
 When you log in for the first time using ssh (using the default root password of 1234), you will be greeted by the screen below.
...
- **Change the root password** and simply follow the initial armbian configuration instructions (when asked to create a new user I created a user called minecraft).
...
- **Prevent the random changing of the ETH MAC address**. 
  ...
  My workaround is to override the MAC address with a fixed one, which can be done by editing the `/etc/network/interfaces` file. Find the eth0 section and add a `hwaddress` stanza below the `iface eth0 inet dhcp` one (observe the indentation!).
  ...
  Having done that, I noticed that somehow the DHCP settings offered by my router were not completely adopted. Specifically, the DNS part was missing! Obviously, there is something broken in the way the NetworkManager operates on this particular armbian image, and I was not inclined to investigate. *Besides, I have yet to see NetworkManager behaving in a sensible way...*

  So, my workaround was  to prevent NetworkManager from touching `/etc/resolv.conf`by putting the `dns=none` into the `[main]` section of the `/etc/NetworkManager/NetworkManager.conf` file.
  ...
  I *could* have gone one step further and completely disabled the NetworkManager, but I did not want to spend the time resolving any potential side-effects.

- **Stop booting into the GUI**

  As the box will only be used over the network, as well as for conserving the limited resources of the box, it makes no sense to boot into the Desktop environment. The easiest way is to use the `armbian-config` ncurses-based configuration tool, as it is just a few steps in the menu.
...
My choice was (later) also corroborated by observing the CPU usage while running the Minecraft server with different implementations. Using OpenJDK 1.8.0_271 resulted in the lowest CPU utilization.
...
You can also choose some other version of OpenJDK, 8, 11, 13, & 14 are available via the default repos.



## Install & run Minecraft server

Obtaining and installing the Minecraft server is relatively easy. The java version of the server can be downloaded [here on the official page](https://www.minecraft.net/en-us/download/server). As of writing this document, the latest release was 1.16.4.

I quickly found out that using the official version consumes lots of resources, especially on the CPU side while the world chunks are being generated. Also during casual play I noticed frequent messages of 

```
[16:00:04 WARN]: Can't keep up! Is the server overloaded? Running 5002ms or 100 ticks behind
```

I've Googled and learned that this message is often received when the server is overloaded.  Which is a bummer, as I only connected with a single client. I then went on to search for a higher performing version. Of course, there is not just one available, but there are a [multitude of community forks](https://www.spigotmc.org/wiki/what-is-spigot-craftbukkit-bukkit-vanilla-forg/) that are at least equivalent to or offer even more features compared to the official version. 

I've settled on [PaperMC](https://papermc.io/), which describes itself as the *'... the next generation of Minecraft server, compatible with Spigot plugins and offering uncompromising performance'.*

Next, I googled for which Java flags to use. I found [this nice page](https://aikar.co/2018/07/02/tuning-the-jvm-g1gc-garbage-collector-flags-for-minecraft/) documenting the flags which are recommended and battle-tested by enthusiasts. As my box has 4 GB of memory, I reserved 3GB for minecraft via `-Xms3G -Xmx3G` flags. Obviously if your box has less or more memory you will need to modify that flag accordingly (always leave at least 100-200MB free for the OS).

Lastly, I run minecraft via screen so it doesn't exit when I logout, and I can easily reattach to it afterwards. The content of my `minecraft.sh` script is shown below.
...
#screen -dm -S Minecraft $java -Xms3G -Xmx3G -jar paper.jar nogui
